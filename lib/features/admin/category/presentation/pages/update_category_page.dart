...
class UpdateCategoryPage extends GetView<CategoryController> {
  ...
  const UpdateCategoryPage({super.key, required this.categoryId});

  @override
  Widget build(BuildContext context) {
    // Find the category to prepopulate
    final category = controller.categories.firstWhereOrNull(
      (c) => c.id == categoryId,
    );
    if (category != null) {
      controller.setForUpdate(category);
    }
                  ...
                  children: [
                    ...
                    Center(
                      child: CustomButton(
                        ...
                        height: 50,
                        onPressed: () => controller.updateCategory(categoryId),
                        text: "Update",
                      ),
                    ),
                  ],
  ...
  Widget _buildImagePicker(BuildContext context) {
    return Obx(() {
      ...
      // Pre-fill with existing URL if no new image selected
      final displayPath =
          image?.path ??
